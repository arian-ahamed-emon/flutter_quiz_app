class AssetPath {
  //Images
  static String _imagePath = 'assets/images';
  static String primaryLogo = '$_imagePath/primary_logo.png';
  static String screenBackground = '$_imagePath/app_bg.jpg';
  static String signInLogo = '$_imagePath/sign_in_logo.png';
  static String facebookLogo = '$_imagePath/facebook_logo.png';
  static String googleLogo = '$_imagePath/google_logo.png';
  static String profileImage = '$_imagePath/profile_image.jpg';

  //quiz bundle
  static String kidsQuiz = '$_imagePath/kids_quiz1.png';
  static String generalQuiz = '$_imagePath/general_banner.png';
  static String programingQuiz = '$_imagePath/programing_banner.png';
  static String sportsQuiz = '$_imagePath/sports_banner.png';
  static String seinceQuiz = '$_imagePath/seience_banner.png';
  static String entertainmentQuiz = '$_imagePath/entertainment_banner.png';
  static String guessQuiz = '$_imagePath/guess_banner.png';

  //Icons
  static String _iconPath = 'assets/icons';
  static String dailyTaskImg = '$_iconPath/task.png';
  static String general= '$_iconPath/book.png';
  static String tech= '$_iconPath/tech1.png';
  static String seince= '$_iconPath/seince.png';
  static String entertainment= '$_iconPath/entertainment.png';
  static String sports = '$_iconPath/sports.png';
  static String travel = '$_iconPath/travel.png';
  static String pgBasic = '$_iconPath/pg_basic.jpg';
  static String flutterBasic = '$_iconPath/flutter_logo.png';
  static String oopLogo = '$_iconPath/oop_logo.jpg';
  static String htmlLogo = '$_iconPath/html_logo.png';
  static String pythonLogo = '$_iconPath/python_logo.jpeg';
}